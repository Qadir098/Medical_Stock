...
public class RecordsModifier {
    ...
    void deleteMedicine(String selectedId, Response objResponse, Connection dbConnection) {
        try{
            PreparedStatement p;// (FirstName,LastName,Title) VALUES (?,?,?);");
   
            p = dbConnection.prepareStatement("delete from medicine where name=? OR Id=?");
     ...
     void updateMedicine(String selectedId, Response objResponse, Connection dbConnection) {
        try{
            PreparedStatement p;// (FirstName,LastName,Title) VALUES (?,?,?);");
   
            p = dbConnection.prepareStatement("update from medicine where name=? OR Id=?");
            p.setString(1, selectedId);
            p.setString(2, selectedId);
            int rowsInserted = p.executeUpdate();
            if(rowsInserted > 0){
                objResponse.messagesList.add(new Message("Medicine updated successfully.", MessageType.Information));
            }
        }catch(SQLException e){
